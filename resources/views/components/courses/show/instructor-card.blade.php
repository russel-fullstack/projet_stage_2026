<div class="bg-white p-6 rounded-2xl shadow-sm border border-slate-100 flex flex-col items-center text-center space-y-4">
    ...
    <div class="relative w-20 h-20">
        ...
        @if($isStudent && $admin)
            ...
            <div class="w-full h-full rounded-full bg-indigo-50 border-2 border-indigo-200 flex items-center justify-center text-indigo-600 font-bold text-xl">
                {{ collect(explode(' ', $admin->name))->map(fn($n) => mb_substr($n, 0, 1))->implode('') }}
            </div>
        @endif
    </div>
    ...
    <!-- Informations -->
    <div>
        <h4 class="font-extrabold text-slate-900 text-sm md:text-base">{{ $admin->name }}</h4>
        <p class="text-xs text-slate-400 font-medium mt-0.5">{{ $admin->role }}</p>
    </div>

    <!-- Bouton de suivi -->
    <button class="w-full flex items-center justify-center gap-2 py-2 px-4 rounded-xl border border-slate-200 hover:border-hover text-slate-600 hover:text-hover font-semibold text-xs transition-all duration-150">
        <!-- Icône d'ajout / suivi -->
