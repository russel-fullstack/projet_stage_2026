...
<form action="{{ route('login') }}" method="POST" class="space-y-4">
    ...
    <!-- Adresse E-mail -->
    <div>
        <label for="email" class="block text-xs font-bold text-[#0c0918] mb-1">Adresse E-mail</label>
        <div class="relative">
            ...
            <input id="email" name="email" type="email" value="{{ old('email') }}"
                ...
                class="w-full pl-10 pr-4 py-2.5 bg-slate-50 border border-slate-200 rounded-2xl text-xs font-medium text-slate-800 placeholder-slate-400 focus:outline-none  focus:ring-2 focus:ring-[#110B29]/10 transition-all"
    ...
    <div>
        <div class="flex items-center justify-between mb-1">
            ...
            @if (Route::has('password.request'))
                <a href="{{ route('password.request') }}"
                    class="text-[11px] font-bold text-[#110B29] hover:underline">Mot de passe oublié ?</a>
            @endif
        </div>
        <div class="relative" x-data="{ show: false }">
            ...
            <button type="button" @click="show = !show"
                ...
                <svg x-show="show" x-cloak class="w-5 h-5" fill="none" stroke="currentColor" stroke-width="2"
                    viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round"
    ...
    <div class="flex items-center justify-between pt-1">
        <label class="flex items-center cursor-pointer">
            <input id="remember_me" name="remember" type="checkbox"
                class="w-4 h-4 rounded border-slate-300 text-[#110B29] focus:ring-[#110B29]/20 cursor-pointer">
...
<p id="passkey-login-message" class="hidden mt-3 text-center text-xs font-bold"></p> --}}
