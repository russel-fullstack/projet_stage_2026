...
<form action="{{ route('login') }}" method="POST" class="space-y-4">
    ...
    <div>
        ...
        <div class="relative" x-data="{ show: false }">
            ...
            <input id="password" name="password"  :type="show ? 'text' : 'password'" placeholder="••••••••" class="w-full pl-10 pr-4 py-2.5 bg-slate-50 border border-slate-200 rounded-2xl text-xs font-medium text-slate-800 placeholder-slate-400 focus:outline-none  focus:ring-2 focus:ring-[#110B29]/10 transition-all"  autocomplete="current-password" />
        <button
            type="button"
            @click="show = !show"
    ...
    <p
        ...
        class="hidden mt-3 text-center text-xs font-bold"
    ></p>
